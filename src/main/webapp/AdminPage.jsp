...
<%
	...
	List<MemberDTO> memberList = dao.memberPage(startPage, onePageCount);
	dao.close();
%>
					...
					<tr align="center">
						...
						<td style="padding:6px 0px 6px 0px">
							<button type="button" class="btn btn-default"
									onclick="location.href='AdminUpdate.jsp?M_ID=<%=dto.getmId() %>&&M_PW=<%=dto.getmPw() %>';"
									style="border:lightgray 1px solid; background:NONE; color:#1E90FF">
									수정</button>
						</td>
						<td style="padding:6px 0px 6px 0px">
							<button type="button" class="btn btn-secondary"
									onclick="location.href='Quit.jsp?M_ID=<%=dto.getmId() %>&&M_PW=<%=dto.getmPw() %>';">
									퇴사</button>
						</td>
					</tr>
				<% } }; %>
