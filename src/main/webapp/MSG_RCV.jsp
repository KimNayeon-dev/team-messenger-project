...
<script>
	...
	function checkImp(obj) {
		var form = document.checkForm;
		var chkMsg = document.getElementsByName("impCheck");
		var check = obj.checked
		
		if(check) {
			console.log(obj.value, "checked");
			form.sendCode.value = obj.value;
			form.action = "CheckImpProcess.jsp";
			form.method = "post";
			form.submit();
		} else {
			console.log(obj.value, "unchecked");
			form.sendCode.value = obj.value;
			form.action = "CheckNmProcess.jsp";
			form.method = "post";
			form.submit();
		}
	}
		    	...
		    	<input type="hidden" name="sendCode">
					...
					<tr align="center">
						...
						<td style="padding-left:4%">
							<label class="checkI">
		    					<input type="hidden" name="sendType" value="<%=dto.getMsgType() %>">
								<input type="hidden" name="impCheckH" value="<%=dto.getMsgImp() %>">
								<input type="checkbox" name="impCheck" id="impCheck" hidden
						...
						<td style="padding:0px">
							<% if(dto.isMsgOpen()) { %>
								<button type="button" class="btn btn-link" style="width:100%; height:49px; color:gray"
...
<script>
	var form = document.checkForm;
	var chkMsgH = document.getElementsByName("impCheckH");
	var chkMsg = document.getElementsByName("impCheck");
	
	for(var i=0; i<chkMsg.length; i++) {
		console.log(chkMsg[i].value, chkMsgH[i].value)
		if(chkMsgH[i].value == "IMP") {
			chkMsg[i].setAttribute('checked', 'checked');
		}
	}
</script>
